// ========== LAYER 2: MEDPASS SECTIONS ==========
// About · Programs (tabbed) · Credentials · Partners · Regions · CTA

import React, { useState } from 'react';
import { Reveal, SectionLabel, SectionHeading, SectionDesc, Button, LevelTag } from '../shared/UI';
import {
  ABOUT_FEATURES, PROGRAM_TABS, PROGRAMS,
  CREDENTIAL_LEVELS, PUBLIC_REGIONS,
} from '../../data/siteData';
...
export const MedPassAbout: React.FC = () => (
  <section id="mp-about" style={{ background: '#F4F6F8', padding: '90px 40px' }}>
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
        ...
        <Reveal>
          <div style={{
            background: '#0B1D3A', borderRadius: 20, padding: 40, overflow: 'hidden',
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18, padding: '20px 0' }}>
              {[
                { label: 'Telth Holding Inc.', sub: 'Parent Organization', parent: true },
                { label: 'Telth-U', sub: 'Academic Programs · Professional Training · R&D · Third-Party Delivery', parent: true },
              ].map((node) => (
                <React.Fragment key={node.label}>
                  <div style={{
                    background: node.parent ? 'rgba(0,180,166,.15)' : 'rgba(255,255,255,.06)',
                    border: `1px solid ${node.parent ? 'rgba(0,180,166,.3)' : 'rgba(255,255,255,.1)'}`,
              ...
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
                {[['MedPass', 'Academic & Training'], ['CME', 'Continued Medical Ed'], ['Licensing', 'Licensing & Language']].map(([h, s]) => (
                  <div key={h} style={{ background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 12, padding: '14px 24px', textAlign: 'center' }}>
                    <h4 style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>{h}</h4>
                    <p style={{ fontSize: 11, color: 'rgba(255,255,255,.5)', marginTop: 3 }}>{s}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Text */}
        <Reveal delay={120}>
          <h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 28, color: '#0A0E1A', marginBottom: 20 }}>
            Where Medical Knowledge Meets Professional Growth
          </h3>
          <p style={{ color: '#6B7890', lineHeight: 1.8, marginBottom: 16, fontSize: 15 }}>
            Telth-U is the education and research arm of Telth Holding Inc., responsible for academic program delivery, professional training, research & development, and third-party program facilitation.
          </p>
          <p style={{ color: '#6B7890', lineHeight: 1.8, marginBottom: 16, fontSize: 15 }}>
            MedPass operates as the dedicated academic and training division within Telth-U, offering structured credential pathways for Care Managers, Collaborative Care Managers, and Physicians worldwide.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 24 }}>
            {ABOUT_FEATURES.map((f) => (
              <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, fontWeight: 500, color: '#0A0E1A' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#00B4A6', flexShrink: 0 }} />
                {f}
              </div>
            ))}
          </div>
...
export const MedPassPrograms: React.FC = () => {
  ...
  return (
    <section id="mp-programs" style={{ background: '#fff', padding: '90px 40px' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <SectionLabel>Program Verticals</SectionLabel>
        <SectionHeading>Training Pathways for Every Healthcare Role</SectionHeading>
        <SectionDesc>
          MedPass delivers specialized programs across three professional verticals — each with four credential levels from foundational certification to advanced fellowship.
        </SectionDesc>

        {/* Tab bar */}
        <div style={{ display: 'flex', gap: 4, background: '#F4F6F8', borderRadius: 14, padding: 5, overflowX: 'auto', marginBottom: 36 }}>
            ...
            <Reveal key={p.title} delay={i * 60}>
              <div style={{
                ...
                onMouseOver={(e) => {
                  ...
                  el.style.borderColor = '#00B4A6';
                  el.style.transform = 'translateY(-4px)';
                  el.style.boxShadow = '0 12px 40px rgba(0,180,166,.08)';
                }}
                onMouseOut={(e) => {
                  const el = e.currentTarget as HTMLDivElement;
              ...
              >
                <div style={{ width: 50, height: 50, borderRadius: 14, background: p.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, marginBottom: 18 }}>
                  {p.icon}
                </div>
...
// ─────────────────────────────────────────────
// CREDENTIALS (four levels)
// ─────────────────────────────────────────────
export const MedPassCredentials: React.FC = () => (
  <section id="mp-levels" style={{ background: '#0B1D3A', padding: '90px 40px', position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      <SectionDesc white>
        Each MedPass program offers a structured progression from foundational knowledge to advanced specialization and thought leadership.
      </SectionDesc>
        ...
        {CREDENTIAL_LEVELS.map((lv, i) => (
          <Reveal key={lv.title} delay={i * 80}>
            <div style={{
              ...
              borderRadius: 20, padding: '34px 26px', textAlign: 'center',
            ...
            >
              ...
              {i < 3 && <span style={{ position: 'absolute', right: -16, top: '50%', transform: 'translateY(-50%)', fontSize: 18, color: 'rgba(255,255,255,.15)' }}>→</span>}
            </div>
          </Reveal>
        ))}
      </div>
...
const PARTNERS_DATA = [
  {
    ...
    desc: 'Partner with Telth-U MedPass to expand your reach and deliver accredited programs through our global education infrastructure.',
    items: [
      { icon: '🏛️', label: 'Medical Schools & Universities' },
      { icon: '🏥', label: 'Hospitals & Clinical Training Centers' },
      { icon: '🔬', label: 'Research Institutions' },
      { icon: '🌐', label: 'Professional Medical Associations' },
    ],
  },
  {
    title: 'For Individual Educators',
    desc: 'Bring your specialized expertise to a global audience of healthcare professionals.',
    items: [
      { icon: '👨‍⚕️', label: 'Subject-Matter Experts & Clinicians' },
      { icon: '📚', label: 'Medical Authors & Researchers' },
      { icon: '🎓', label: 'CME Instructors & Trainers' },
      { icon: '💡', label: 'Healthcare Technology Innovators' },
    ],
  },
];

export const MedPassPartners: React.FC = () => (
  <section id="mp-partners" style={{ background: '#F4F6F8', padding: '90px 40px' }}>
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      <SectionLabel>Partnerships</SectionLabel>
      <SectionHeading>Open Platform for Third-Party Providers</SectionHeading>
      <SectionDesc>
        We welcome institutions and individual educators to deliver training and academic programs through the MedPass platform.
      </SectionDesc>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {box.items.map((item) => (
                  <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', background: '#F4F6F8', borderRadius: 10, fontSize: 13, fontWeight: 500, color: '#0A0E1A' }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(0,180,166,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}>
                      {item.icon}
                    </div>
                    {item.label}
                  </div>
...
export const MedPassRegions: React.FC = () => (
  <section id="mp-regions" style={{ background: '#fff', padding: '90px 40px' }}>
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 22 }}>
        {PUBLIC_REGIONS.map((r, i) => (
          <Reveal key={r.name} delay={i * 80}>
            <div style={{
              background: '#F4F6F8', borderRadius: 20, padding: '34px 26px',
...
export const MedPassCTA: React.FC = () => (
  <section id="mp-contact" style={{ background: '#0B1D3A', padding: '90px 40px', position: 'relative', overflow: 'hidden' }}>
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
      <h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 'clamp(28px,4vw,44px)', color: '#fff', marginBottom: 18, lineHeight: 1.2 }}>
